...
class SeparableEnv(gym.Env):
    ...
    This interface is superficially similar to :py:class:`gym.GoalEnv`,
    but doesn't pose any requirements to the observation space. (By
    contrast, :py:class:`gym.GoalEnv` requires that the observation
    space is a dict with keys ``"observation"``, ``"desired_goal"`` and
    ...
    - replacing :py:meth:`compute_observation()` with a function
      approximator, e.g. a neural network;
    - estimating the goodness of the very initial observation of an
      episode via ``env.compute_reward(env.reset(), {})``.

    Because of these use cases, all state transition should be
    restricted to :py:meth:`compute_observation()`. In particular, it
    must be possible to call :py:meth:`compute_reward()` and
    :py:meth:`compute_done()` multiple times without changing the
    internal state of the environment.
    """

    def step(self, action: np.ndarray) -> t.Tuple[np.ndarray, float, bool, InfoDict]:
        """Implementation of :py:meth:`gym.Env.step()`.

        This calls in turn the three new abstract methods:
        :py:meth:`compute_observation()`, :py:meth:`compute_reward()`,
        and :py:meth:`compute_done()`.
        """
        info: InfoDict = {}
    ...
    def compute_observation(self, action: np.ndarray, info: InfoDict) -> np.ndarray:
        ...
        :py:meth:`compute_observation()`, the other two compute methods
        can be called as often as desired and always give the same
        results, given then the same arguments.

        Args:
            action: the action that was passed to :py:meth:`step()`.
            info: an info dictionary that may be filled with
                additional information.

        Returns:
            The next observation to be returned by :py:meth:`step()`.
        """
    ...
    def compute_reward(self, obs: np.ndarray, goal: None, info: InfoDict) -> float:
        ...
        similar to :py:meth:`gym.GoalEnv.compute_reward()`, but it
        doesn't impose any structure on the observation space.

        Note that this function should be free of side-effects or
        modifications of ``self``. In particular, the user is allowed to
        do multiple calls to ``env.compute_reward(obs, {})`` and always
        get the same result.

        Args:
            obs: The observation calculated by :py:meth:`reset()` or
                :py:meth:`compute_observation()`.
            goal: A dummy parameter to stay compatible with the
                :py:class:`gym.Env` API. This parameter generally is
                None. If you want a multi-goal environment, consider
                :py:class:`SeparableGoalEnv`.
            info: an info dictionary with additional information.
                It may or may not have been passed to
                :py:meth:`compute_observation()` before.

        Returns:
            float: the reward that corresponds to the given observation.
                This value is returned by :py:meth:`step()`.
        """
        raise NotImplementedError()
...
class SeparableOptEnv(SeparableEnv, SingleOptimizable):
    ...
    This is an intersection of :py:class:`SeparableEnv` and
...
class SeparableGoalEnv(gym.GoalEnv):
    ...
    :py:class:`SeparableEnv` in the meaning of the parameters that are
    passed to :py:meth:`compute_reward()`.
    ...
    Because of these use cases, all state transition should be
    restricted to :py:meth:`compute_observation()`. In particular, it
    must be possible to call :py:meth:`gym.GoalEnv.compute_reward()` and
    :py:meth:`compute_done()` multiple times without changing the
    internal state of the environment.
    """
    ...
    def compute_done(self, obs: GoalObs, reward: float, info: InfoDict) -> bool:
        ...
        Args:
            obs: The observation calculated by :py:meth:`reset()` or
                :py:meth:`compute_observation()`.
            reward: The observation calculated by
                :py:meth:`gym.GoalEnv.compute_reward()`.
            info: an info dictionary with additional information. It may
                or may not have been passed to
                :py:meth:`gym.GoalEnv.compute_reward()` before.
...
class SeparableOptGoalEnv(SeparableGoalEnv, SingleOptimizable):
    ...
    This is an intersection of :py:class:`SeparableGoalEnv` and
    :py:class:`SingleOptimizable`. Any class that inherits from both,
    also inherits from this class.
    """
